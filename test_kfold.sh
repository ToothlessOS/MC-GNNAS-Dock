...
do
    ...
    version_dir="lightning_logs/version_$((150+$i))/checkpoints"
    ...
    if [ -d "$version_dir" ]; then
        ...
        for ckpt_file in "$version_dir"/last.ckpt; do
            ...
            if [ -f "$ckpt_file" ]; then
                ...
                python main.py --k_folds 10 --fold_num $i --model_name rank --max_epochs 500 --loss bce --devices 0 --seed 42 --no_ndcg_loss --no_logistic_loss --test --ckpt_path "$ckpt_file" --num_classes 4 --drop_columns smina diff diffL karma
